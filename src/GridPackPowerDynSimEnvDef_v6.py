...
class GridPackPowerDynSimEnv(gym.Env):
    ...
    def __init__(self, simu_input_file, rl_config_file, force_symmetric_continuous_action=False,
        ...
        self.otherzones_action_buses = self.action_buses[-self.actionLoadOtherZoneNum:]
        self.otherzones_action_ranges = self.action_ranges[-self.actionLoadOtherZoneNum:,:]
        self.otherzones_action_buses_pvalue_pu = self.action_buses_pvalue_pu[-self.actionLoadOtherZoneNum:]
    ...
    def step(self, action):
        ...
        if self.apply_uvls_otherzones:
            for idxtmp in range(len(self.otherzones_action_buses)):
                ...
                if volt_tmp < 0.7 and ( 0.0 <(self.current_simu_time-0.23-fault_end_time)<self.simu_time_step):
                    loadshedact.actiontype = 0
                ...
                if volt_tmp < 0.8 and ( 0.0 <(self.current_simu_time-0.4-fault_end_time)<self.simu_time_step):
                    loadshedact.actiontype = 0
                    loadshedact.bus_number = ibus
                    loadshedact.componentID = "1"
                    loadshedact.percentage = self.otherzones_action_ranges[idxtmp, 0]
                ...
                if volt_tmp < 0.9 and ( 0.0 <(self.current_simu_time-1.0-fault_end_time)<self.simu_time_step):
                    loadshedact.actiontype = 0
                    loadshedact.bus_number = ibus
                    loadshedact.componentID = "1"
                    loadshedact.percentage = self.otherzones_action_ranges[idxtmp, 0]
                ...
                if volt_tmp < 0.91 and ( 0.0 <(self.current_simu_time-2.0-fault_end_time)<self.simu_time_step):
                    loadshedact.actiontype = 0
                    loadshedact.bus_number = ibus
                    loadshedact.componentID = "1"
                    loadshedact.percentage = self.otherzones_action_ranges[idxtmp, 0]
                ...
                if volt_tmp < 0.92 and ( 0.0 <(self.current_simu_time-3.0-fault_end_time)<self.simu_time_step):
                    loadshedact.actiontype = 0
                    loadshedact.bus_number = ibus
                    loadshedact.componentID = "1"
                    loadshedact.percentage = self.otherzones_action_ranges[idxtmp, 0]
